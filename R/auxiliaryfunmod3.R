...
#------------------------------------------------------------------------
disableagerglm <- function(data, family, dist, design_matrix, fixparam, initparam, argumdist, opt, hyperparameters, maxiter, tolerance, np, link_function, ydist, optimizer) {
        ...
        while(TRUE){
                ...
                if (is.na(loss[[step]])){
                        stop(paste0("The process failed because the loss value in the last iterarion is NaN \n",
                                    ...
                                    "integer nor float. \n",
                                    "3. Change the initial values provided for the coefficients. \n",
