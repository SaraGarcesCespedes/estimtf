...
estimation_2 <- dist_estimtf2(x = x, xdist = pdf, initparam =                                     list(lambda = rnorm(1, 2, 1)),                                      optimizer = "AdamOptimizer",
                              hyperparameters = list(learning_rate =                               0.1), maxiter = 10000)
...
formula <- dist ~ speed
estimation_1 <- estimglm(formula, family = "Normal", 
...
summary(estimation_1)
predict(estimation_1, newdata = data.frame(speed = c(10, 40, 50)))
```
